...
const swaggerAuthPath = join(process.cwd(), 'data', 'swagger-auth.json');
...
async function generateTypes() {
  ...
  console.log('Reading swagger-auth.json...');
  const swaggerAuthContent = readFileSync(swaggerAuthPath, 'utf-8');
  const swaggerAuth = JSON.parse(swaggerAuthContent);
  
  // Merge definitions from both files
  ...
  if (Object.keys(allDefinitions).length === 0) {
    console.error('No definitions found in swagger files');
    return;
  }
  
  console.log(`Found ${Object.keys(swagger.definitions || {}).length} definitions in swagger.json`);
  console.log(`Found ${Object.keys(swaggerAuth.definitions || {}).length} definitions in swagger-auth.json`);
  console.log(`Total: ${Object.keys(allDefinitions).length} definitions`);
...
`;
  ...
  const sortedDefinitions = Object.entries(allDefinitions).sort(([a], [b]) => {
    const aHasRef = JSON.stringify(allDefinitions[a]).includes('$ref');
    const bHasRef = JSON.stringify(allDefinitions[b]).includes('$ref');
  ...
  console.log(`Total definitions processed: ${Object.keys(allDefinitions).length}`);
}
